const routerName = 'phiVanChuyen';
const renderName = `backend/page/${routerName}/`;

const linkPrefix = `/dhuy782/phiVanChuyen/`
...
module.exports = {
    getlist : async (req , res , next) => {
        ...
        let { data }  = await phiVanChuyenService.getAll({pagination, condition, sort})

        let choosedStatus = req.params.status;
        let statusFilter = await phiVanChuyenService.countAll({choosedStatus})

        let pageTitle = 'Phí Vận Chuyển'
    ...
    getForm : async (req , res , next) => {
        ...
        let { pageTitle, data } = await phiVanChuyenService.getForm({id})
    ...
    getStatus: async (req , res , next) => {
        ...
        let data = await phiVanChuyenService.changeStatus({id, status})
        data.status        = status
        data.id            = id
    ...
    //     let {success} = await phiVanChuyenService.changeNumber({id, data})
    ...
    //     await phiVanChuyenService.deleteItem({id})
    ...
    saveItem: async (req, res, next) => {
        ...
        if(typeof item !== 'undefined' && item.id !== ""){ //edit
            phiVanChuyenService.editItem(item)
            req.flash('success', notify.EDIT_SUCCESS, false) 
            res.redirect(`${linkPrefix}`)
        }else{ // add
            phiVanChuyenService.addItem(item)
    ...
    changeMultipleAction: async (req, res, next) => {
        ...
        if (action === 'delete') {
            let { deletedCount } = await phiVanChuyenService.ChangeDeleteMultiple({arrId})
            req.flash('success', util.format(notify.DELETE_MULTI_SUCCESS, deletedCount), false) 
            res.redirect(`${linkPrefix}`)
        }else{
            let { modifiedCount } = await phiVanChuyenService.ChangeStatusMultiple({arrId, action})
